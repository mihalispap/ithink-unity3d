...
{
    private const float VERSION = 2.1F;
    ...
    void Awake()
    {
        ...
        _windowRect = new Rect( 30.0F, 30.0F, 400.0F, 500.0F );
    ...
    private void MainWindow( int windowID )
    {
        ...
        else if ( _mode == Mode.CopyLog )
        {
            ...
            if ( GUILayout.Button( "Copy to Clipboard", GUILayout.Width( 95.0F ), GUILayout.Height( 20.0F ) ) )
            {
                System.IO.TextWriter fileWriter;
                string fileName = Application.persistentDataPath + "/iThinkPlanner.log";
                DebugConsole.Log( fileName );
                fileWriter = System.IO.File.CreateText( fileName );
                fileWriter.WriteLine( _displayString.ToString() );
                fileWriter.Close();
            }
