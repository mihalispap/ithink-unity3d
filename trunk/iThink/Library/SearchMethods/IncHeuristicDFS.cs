...
namespace iThinkLibrary{
	...
	public class IncHeuristicDepthFS : iThinkPlanner
	{
		public iThinkActionManager actionManager;

		public IncHeuristicDepthFS() : base(){ depth = 7; }

		public override iThinkPlan SearchMethodBounded( iThinkState GoalState, iThinkActionManager ActionManager, List<iThinkPlan> OpenStates, List<iThinkState> VisitedStates, double timelimit = 10 )
		{
			...
			while ( OpenStates.Count != 0 )
			{
				...
				if (curStep.getActionCount() < depth)
				{
					...
					/************************************
					 *  GET AVAILABLE ACTIONS FOR HDFS  *
					 ************************************/
					...
					if ( OpenStates.Count > 0 ) {
						Plan = new iThinkPlan(OpenStates[0]);
					}
					TimeSpan timediff = DateTime.Now  - n1;
					if ( timediff.TotalSeconds > timelimit )
					{
						Plan.getState().ToString();
						return Plan;
					}
					++it;
				}
